...
import {
  ...
  purgeInvalidAndDuplicateCatalogPairs
} from "../utils/suggestions-catalog";
...
class SuggestionsService {
  ...
  private countCategoriesInExistingCatalogRows(rows: DbRecipeRow[]): {
    desserts: number;
    fish: number;
    dairy: number;
  } {
    ...
    for (const row of rows) {
      const pairKey = String((row as any).pairKey ?? "").trim();
      if (!pairKey || countedPairs.has(pairKey)) continue;

      const recipe = mapDbRowToFullRecipe(row);
      const categories = normalizeCategories(recipe.categories ?? recipe.data?.categories ?? []);

      if (categories.includes("deserts" as any)) desserts++;
      if (categories.includes("fish" as any)) fish++;
      if (categories.includes("dairy" as any)) dairy++;

      countedPairs.add(pairKey);
    }

    return { desserts, fish, dairy };
  }

  private pickIdeaForMissingCategory(
    existingCounts: { desserts: number; fish: number; dairy: number }
  ): string | null {
    const dessertTarget = 6;
    const fishTarget = 6;
    const dairyTarget = 6;

    if (existingCounts.desserts < dessertTarget) {
      const dessertIdeas = [
      ...
      return dessertIdeas[Math.floor(Math.random() * dessertIdeas.length)];
    }

    if (existingCounts.fish < fishTarget) {
      const fishIdeas = [
      ...
      return fishIdeas[Math.floor(Math.random() * fishIdeas.length)];
    }

    if (existingCounts.dairy < dairyTarget) {
      const dairyIdeas = [
      ...
      return dairyIdeas[Math.floor(Math.random() * dairyIdeas.length)];
    }
  ...
  private async insertSuggestionRecipe(args: {
    ...
    const queryRestrictionsJson = JSON.stringify(sanitizeQueryRestrictions(r.queryRestrictions ?? []));
    const categoriesJson = JSON.stringify(normalizeCategories(r.categories ?? r.data?.categories ?? []));
...
`.trim();
  ...
  public async generateOnce(): Promise<{ createdPairs: number; createdRows: number }> {
    ...
    try {
      ...
      await purgeInvalidAndDuplicateCatalogPairs(systemUserId);
      ...
      const { usedTitles, usedContentHashes } = await getUsedKeys(systemUserId);
      const ideas = getIdeas("en");

      const currentRows = (await dal.execute(
        `select * from recipe where userId = ? and pairKey is not null order by id asc`,
        [systemUserId]
      )) as DbRecipeRow[];

      const existingCategoryCounts = this.countCategoriesInExistingCatalogRows(currentRows);
      ...
      for (let attempts = 0; createdPairs < missingPairs && attempts < 600; attempts++) {
        try {
          const prioritizedIdea = this.pickIdeaForMissingCategory(existingCategoryCounts);
          const query = prioritizedIdea ?? ideas[Math.floor(Math.random() * ideas.length)];
          const input = this.createKosherInputModel(query);
          const data = await recipeService.generateInstructions(input, false);

          const normalizedTitleValue = normalizeTitle(data.title);
          if (!normalizedTitleValue || usedTitles.has(normalizedTitleValue)) continue;
          ...
          if (await hasExistingSuggestion(systemUserId, normalizedTitleValue, contentHash)) continue;
          ...
          const qrWithHash = sanitizeQueryRestrictions(data.queryRestrictions);
          qrWithHash.push(`__CONTENT_HASH__:${contentHash}`);

          const naturalEnTitle = naturalizeRecipeTitle(data.title);

          const en = new FullRecipeModel({
            title: naturalEnTitle,
            amountOfServings: data.amountOfServings,
            description: data.description,
          ...
          const heQrWithHash = sanitizeQueryRestrictions(heJson.queryRestrictions);
          heQrWithHash.push(`__CONTENT_HASH__:${contentHash}`);

          const naturalHeTitle = naturalizeRecipeTitle(heJson.title);

          const he = new FullRecipeModel({
            title: naturalHeTitle,
          ...
          usedTitles.add(normalizedTitleValue);
          usedContentHashes.add(contentHash);
          createdPairs++;
          createdRows += 2;

          if (normalizedCategories.includes("deserts" as any)) {
            existingCategoryCounts.desserts++;
          }
          if (normalizedCategories.includes("fish" as any)) {
            existingCategoryCounts.fish++;
          }
          if (normalizedCategories.includes("dairy" as any)) {
            existingCategoryCounts.dairy++;
          }
        } catch {
          continue;
        }
