...
class VisitorRecipeUsageService {
  private readonly max_amount_per_usage = 5;
  ...
  public async getStatus(visitorId: string): Promise<{ used: number; remaining: number; windowEndsAt: Date | null }> {
    ...
    if (rows.length === 0) return { used: 0, remaining: this.max_amount_per_usage, windowEndsAt: null };

    const used = rows[0].used ?? 0;
    return { used, remaining: Math.max(0, this.max_amount_per_usage - used), windowEndsAt: rows[0].windowEndsAt };
  }
}
